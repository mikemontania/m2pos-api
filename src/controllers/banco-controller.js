...
const getById = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al buscar el banco por ID' });
  }
};
...
const findAll = async (req, res) => {
  try {
    ...
    const bancos = await Banco.findAll({ where: conditions });
...
const disable = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al desactivar el banco' });
  }
};
