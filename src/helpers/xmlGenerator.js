...
const generaXML = async (cabecera, detalles) => {
  ...
  const moneda = const_monedas.find(
    t => t.codigo == cabecera.empresa.codigoMoneda
);
...
const createGTotSub = cabecera => {
  return {
    gTotSub: {
      ...
      dBaseGrav5:
        cabecera.importeIva5 > 0
          ? cabecera.importeTotal - roundTo8Decimals(cabecera.importeIva5)
          : 0,
      dBaseGrav10:
        cabecera.importeIva10 > 0
          ? cabecera.importeTotal - roundTo8Decimals(cabecera.importeIva10)
          : 0,
      dTBasGraIVA:
        cabecera.importeIva5 > 0 || cabecera.importeIva10 > 0
          ? cabecera.importeTotal -
            roundTo8Decimals(cabecera.importeIva5 + cabecera.importeIva10)
          : 0
    }
  };
};
